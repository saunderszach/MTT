...
We also note that the range of $\rho$ values corresponding to a single value of $\sigma$ decreases as the number of targets increases. In other words, as the number of targets increases, it becomes more difficult to generate a wider variety of $\rho$ values from a given $\sigma$. In particular, it becomes increasingly difficult to generate easier scenarios for when there are ten targets, which can be seen by the fact that $\rho$ is more heavily weighted toward zero for $\sigma \leq 1.0$ when $P=10$ than when $P=4$. Most likely, this is a result of using a fixed state space. As the number of targets increases, the ability to separate this targets within the fixed state space decreases, meaning that all of the trajectories will lie close to each other. 
...
Both the heuristic and the MIO model scale well increases in the number of targets. For scenarios with four targets, all solutions are closely comparable to the ideal solution estimates, while for scenarios with larger numbers of targets, there appears to be a lower bound to performance, even as $\sigma$ increases. In particular, the gap between the ideal estimation error and the heuristic/MIO estimation error remains relatively constant across all values of $P$ for a fixed number of scans. 

\mysubsubsection{Summary}
We have shown that in the case of no detection ambiguity, the heuristic is highly scalable, and it finds good quality solutions in fractions of a second. Using these solutions as a warm start, the MIO achieves high quality solutions to both the data association and trajectory estimation problems after T or fewer seconds. Furthermore, the MIO is scalable with respect to increases in both the number of targets and scans. We have also identified that there exists a tradeoff between making correct data associations and estimating trajectories more closely. Particularly in cases of high signal to noise ratios, it may be necessary to sacrifice incorrect data associations in order to improve the trajectory estimates. With these points in mind, we advance to discussing the case of scenarios with detection ambiguity. 
...
As expected, the robust heuristic requires longer run times than the basic heuristic, due to the increase in combinatorial solutions in the robust heuristic over the basic heuristic. Comparing Table~\ref{tab:Basic_heuristic_times} and Table~\ref{tab:Robust_heuristic_times} we see that robust run times for four estimated targets are roughly four times as long as corresponding run times in the basic heuristic for a fixed number of scans. However, the magnitude of this effect appears to decay as the number of targets increases. In particular, further examination of run times for eight targets in both heuristic indicates that the robust heuristic times are only about twice that of the basic heuristic. This suggests that the robust heuristic still scales well with increases in the number of targets. 

The robust heuristic scales extremely well with respect to $P_{\text{est}}$ More importantly, scalability actually improves as the number of estimated targets and scans increases. To demonstrate, increasing from two to six estimated targets for four scans roughly doubles the run time, while increasing from eight to twelve estimated targets for four scans increases the run time by only 12\%. In fact, for four scans, increasing from two estimated targets to twelve only increases the average run time by a total of $143\%$. This result suggests that the robust heuristic run times scale efficiently in regards to $P_{\text{est}}$. However, the robust heuristic does not scale as well in regards to $T$, roughly doubling in run time with each increase of two scans for a fixed $P_{\text{est}}$. 

The robust heuristic does exhibit a higher risk of worst case potential. Whereas the maximum times for the basic heuristic were not exceedingly longer than the average run times, the robust heuristic is more at risk to high run times, with worst case run times exceeding 75 milliseconds. However, the average run times clearly remain on the order of milliseconds, with the largest average run time being 7.7 milliseconds. Thus, the robust heuristic remains highly scalable in regards to run time, though its risk of worst case run times is admittedly higher. 

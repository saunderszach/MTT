...
\item At each point in time the sensor generates false alarms
	according to a Poisson distribution with rate $\lambda_{FA}$, which are located uniformly in the space.
...
%\begin{enumerate}[(i)]
%\item The sensor does not generate a detection for any target for\\
%	 any time with probability $P_{d}$ which is constant and\\
%	 independent between targets and scans.
%\item At each point in time the sensor generates false alarms\\
%	according to a Poisson distribution with rate $\lambda_{FA}$,\\
%	which are located uniformally in the space.
%\item The number of true targets $P$ satisfies $N_0\leq P \leq N_1$.
%\end{enumerate}
%\end{minipage}
%\end{assumption}
...
\begin{align*}
\underset{\psi_{jt}}{\text{minimize: }} & \sum_{j=1}^{N_{1}} \sum_{t=1}^{T} \psi_{jt} + \theta TF + \phi TM\\

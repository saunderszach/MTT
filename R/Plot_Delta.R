...
Sigma_Data = Data %>% filter(Solution_Type != "Ideal", N == "10000") %>% group_by(P,T,Sigma,Solution_Type) %>% summarize(mean.value=mean(Delta),max.value=max(Delta),min.value=min(Delta))
...
plot_2 = ggplot(data = Heuristic_Data,
  ...
  geom_jitter() +
  facet_grid(P~T, labeller=label_both) +
  ggtitle(paste("Heuristic Delta vs Sigma by N")) +
  theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18,face="bold"),
        plot.title=element_text(size = rel(2)),
        legend.text=element_text(size=14),
...
MIO_Data = Data %>% filter(Solution_Type=="Optimized t=3T", N=="10000") %>% group_by(P,T,Sigma,Scenario_Type) %>% summarize(mean.value=mean(Delta),max.value=max(Delta),min.value=min(Delta))
...
plot_3 = ggplot(data = MIO_Data,
  ...
  geom_jitter() +
  facet_grid(P~T, labeller=label_both) +
  ggtitle(paste("MIO Delta vs Sigma by Scenario Type")) +
  theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18,face="bold"),
        plot.title=element_text(size = rel(2)),
        legend.text=element_text(size=14),
...
MIO_Data2 = Data %>% filter(Solution_Type=="Optimized t=3T") %>% group_by(P,T,Sigma,N) %>% summarize(mean.value=mean(Delta),max.value=max(Delta),min.value=min(Delta))
...
plot_4 = ggplot(data = MIO_Data2,
  ...
  geom_jitter() +
  facet_grid(P~T, labeller=label_both) +
  ggtitle(paste("MIO Delta vs Sigma by N")) +
  theme(axis.text=element_text(size=16),
        axis.title=element_text(size=18,face="bold"),
        plot.title=element_text(size = rel(2)),
        legend.text=element_text(size=14),
...
Save_str = paste("Plots/Delta/vs_Sigma/MIO_only_by_N.png", sep="")
png(file=Save_str,width=1000, height=700)
print(plot_4)
dev.off()
...
for (type in Rho_Types){
  ...
  Rho_Data2 = Rho_Data %>% filter(N=="10000") %>% group_by(P,T,Rho,Solution_Type) %>% summarize(mean.value=mean(Delta),max.value=max(Delta),min.value=min(Delta))
  ...
  plot_2 = ggplot(data = Heuristic_Data,
    ...
    geom_ribbon(data=Heuristic_Data,
                ...
                alpha=.1) +
    facet_grid(P~T, labeller=label_both) +
    ggtitle(paste("Heuristic Delta vs", type, "by N")) +
    theme(axis.text=element_text(size=16),
          axis.title=element_text(size=18,face="bold"),
          plot.title=element_text(size = rel(2)),
          legend.text=element_text(size=14),
  ...
  MIO_Data = Rho_Data %>% filter(Solution_Type=="Optimized t=3T", N=="10000") %>% group_by(P,T,Rho,Scenario_Type) %>% summarize(mean.value=mean(Delta),max.value=max(Delta),min.value=min(Delta))
  ...
  plot_3 = ggplot(data = MIO_Data,
    ...
    geom_ribbon(data=MIO_Data,
                ...
                alpha=.1) +
    facet_grid(P~T, labeller=label_both) +
    ggtitle(paste("MIO Delta vs", type, "by Scenario Type")) +
    theme(axis.text=element_text(size=16),
          axis.title=element_text(size=18,face="bold"),
          plot.title=element_text(size = rel(2)),
          legend.text=element_text(size=14),
  ...
  MIO_Data2 = Rho_Data %>% filter(Solution_Type=="Optimized t=3T", N=="10000")  %>% group_by(P,T,Rho,N) %>% summarize(mean.value=mean(Delta),max.value=max(Delta),min.value=min(Delta))
  ...
  plot_4 = ggplot(data = MIO_Data2,
    ...
    geom_ribbon(data=MIO_Data2,
                ...
                alpha=.1) +
    facet_grid(P~T, labeller=label_both) +
    ggtitle(paste("MIO Delta vs", type, "by N")) +
    theme(axis.text=element_text(size=16),
          axis.title=element_text(size=18,face="bold"),
          plot.title=element_text(size = rel(2)),
          legend.text=element_text(size=14),
